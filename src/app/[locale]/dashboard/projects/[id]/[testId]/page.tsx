...
export default async function TestRunnerPage({ params }: Props) {
  ...
  // Assegurem que el test pertany al projecte de la URL (evitar manipulació d'URL)
  if (ctx.campaign.projectId !== projectId) return notFound();

  // 3. Càlcul de Gamificació (Progrés)
  const totalTasks = ctx.tasks.length;
  // ✅ CORRECCIÓ: Definim la variable correctament
  const completedTasks = ctx.results.filter(r => r.status === 'pass').length;
  
  // ✅ CORRECCIÓ: Utilitzem 'completedTasks' aquí
  const progress = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;

  // Missatge motivacional segons progrés
  let motivation = "Comencem la missió! 🚀";
  if (progress > 0) motivation = "Bon començament! Continua així. 💪";
  if (progress > 50) motivation = "Ja has passat la meitat! Ets un crack. 🔥";
  if (progress === 100) motivation = "MISSIÓ COMPLERTA! 🎉 Gràcies pel teu temps.";
      ...
      <div className="bg-slate-900 border-b border-slate-800 sticky top-0 z-10 shadow-lg">
        <div className="max-w-6xl mx-auto px-4 py-4">
            ...
            <div className="flex items-center justify-between mb-4">
                {/* Botó enrere amb paràmetre source=project per tornar a la llista correcta */}
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:sticky lg:top-36 space-y-6">
                ...
                {progress === 100 && (
                    <div className="bg-green-500/10 border border-green-500/20 p-6 rounded-xl text-center animate-in zoom-in">
                        ...
                        <p className="text-sm text-green-700 dark:text-green-300">Has completat totes les tasques d'aquesta missió.</p>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-4">
            <h2 className="text-lg font-bold flex items-center gap-2 mb-4">
                ...
                Llista de Verificació ({completedTasks}/{totalTasks})
            </h2>
