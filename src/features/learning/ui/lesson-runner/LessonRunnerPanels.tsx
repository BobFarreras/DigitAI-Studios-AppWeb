...
export function LessonResult({ data, score, xp }: { data: LearningRunnerData; score: number; xp: number }) {
  ...
  return (
    <section className="mx-auto max-w-md py-10 text-center">
      ...
      <h1 className="mt-6 text-3xl font-black text-[#3c3c3c] dark:text-white">{t('lesson_completed_title')}</h1>
      <p className="mt-3 text-lg font-black text-[#777777]">{score}% {t('lesson_accuracy')}{xp} XP</p>
      <Button asChild className="mt-8 h-12 rounded-xl bg-[#58cc02] font-black text-white shadow-[0_5px_0_#3f8f01]">
        <Link href={`/dashboard/learn/${data.trackSlug}`}>{t('lesson_back_to_map')}</Link>
      </Button>
    </section>
