...
import { addTesterAction, removeTesterAction } from '../actions/admin-actions';
import { Button } from '@/components/ui/button';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { Trash2, UserPlus, Search, Check } from 'lucide-react';
import { toast } from 'sonner'; // O el teu sistema d'alertes
...
export function TesterManager({ campaignId, assignedTesters, availableTesters }: Props) {
  ...
  // Filtrem els usuaris que NO estan assignats encara
  const filteredCandidates = availableTesters
    .filter(u => !assignedTesters.some(at => at.id === u.id))
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                ...
                {assignedTesters.map(tester => (
                    <div key={tester.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                        <div className="flex items-center gap-3">
                            <Avatar className="h-8 w-8">
                                <AvatarImage src={tester.avatar_url || ''} />
                                <AvatarFallback>{tester.email[0].toUpperCase()}</AvatarFallback>
                            </Avatar>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800 p-6">
            ...
            <div className="relative mb-4">
                ...
                <input 
                    ...
                    className="w-full pl-9 p-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-primary outline-none"
                />
            </div>

            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                {filteredCandidates.length === 0 ? (
                    <p className="text-sm text-slate-400 text-center py-4">No s'han trobat usuaris.</p>
                ) : (
                    filteredCandidates.map(user => (
                        <div key={user.id} className="flex items-center justify-between p-2 hover:bg-white dark:hover:bg-slate-800 rounded-lg transition-colors group cursor-pointer" onClick={() => handleAdd(user.id)}>
                            <div className="flex items-center gap-3">
                            ...
                            <Button size="sm" variant="ghost" className="opacity-0 group-hover:opacity-100 text-blue-600" disabled={isPending}>
                                Assignar
                            </Button>
                        </div>
                    ))
