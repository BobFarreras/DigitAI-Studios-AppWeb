      ...
      <section className="flex h-full flex-col">
        <div className="flex min-h-12 items-center justify-between border-b border-[#d0d6e0] px-4 dark:border-[#23252a]">
          <div className="flex items-center gap-1 text-[12px] font-[560]">
            <Tab active={stateFilter === 'all'} onClick={() => setStateFilter('all')} label={ui.t('all')} />
            {states.map((s) => <Tab key={s} active={stateFilter === s} onClick={() => setStateFilter(s)} label={ui.state(s)} />)}
          </div>
          <div className="flex items-center gap-2">
            <label className="hidden h-8 items-center gap-2 rounded-[6px] border border-[#c0c8d5] bg-white px-2 text-[12px] dark:border-[#323334] dark:bg-[#08090a] md:flex">
              ...
              <input value={jobTitle} onChange={(e) => onSetJobTitle(e.target.value)} placeholder={ui.t('search')} className="w-32 bg-transparent outline-none placeholder:text-[#8a8f98]" />
            </label>
            <SlidersHorizontal className="h-4 w-4 text-[#8a8f98]" />
            <button onClick={() => setOpenDialog(true)} className="inline-flex h-8 items-center justify-center gap-2 rounded-[6px] bg-[#08090a] px-3 text-[12px] font-semibold text-white dark:bg-[#e4f222] dark:text-[#08090a]"><Plus className="h-4 w-4" />{ui.t('create')}</button>
          </div>
        </div>
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center rounded-[8px] bg-white/62 p-4 backdrop-blur-md dark:bg-[#08090a]/70">
          <div className="max-h-full w-full max-w-2xl overflow-auto rounded-[10px] border border-[#d0d6e0] bg-[#f7f8f8] p-4 text-[#08090a] shadow-[0_24px_80px_rgba(8,9,10,0.22)] dark:border-[#323334] dark:bg-[#0f1011] dark:text-[#f7f8f8]">
            <div className="mb-3 flex items-start justify-between gap-3 border-b border-[#d0d6e0] pb-3 dark:border-[#23252a]">
              <div><h4 className="text-[16px] font-semibold">{ui.t('createSatOrder')}</h4><p className="mt-1 text-[12px] text-[#62666d] dark:text-[#8a8f98]">{ui.t('satDialogHelp')}</p></div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field value={form.title} onChange={(v) => setForm((f) => ({ ...f, title: v }))} label={ui.t('issueSummary')} />
              <Field value={form.client} onChange={(v) => setForm((f) => ({ ...f, client: v }))} label={ui.t('client')} />
              <Field value={form.technician} onChange={(v) => setForm((f) => ({ ...f, technician: v }))} label={ui.t('assignedTech')} />
              <Field value={form.eta} onChange={(v) => setForm((f) => ({ ...f, eta: v }))} label={ui.t('eta')} />
              <Field value={form.contact} onChange={(v) => setForm((f) => ({ ...f, contact: v }))} label={ui.t('contact')} />
              <Field value={form.location} onChange={(v) => setForm((f) => ({ ...f, location: v }))} label={ui.t('location')} />
              <Select label={ui.t('type')} value={form.type} onChange={(v) => setForm((f) => ({ ...f, type: v as JobType }))} values={['Reparacio', 'Manteniment', 'Muntatge', 'Auditoria']} format={(v) => ui.type(v as JobType)} />
              <Select label={ui.t('priority')} value={form.priority} onChange={(v) => setForm((f) => ({ ...f, priority: v as JobPriority }))} values={['Alta', 'Mitja', 'Baixa']} format={(v) => ui.priority(v as JobPriority)} />
              <Select label="SLA" value={form.sla} onChange={(v) => setForm((f) => ({ ...f, sla: v as JobSla }))} values={['OK', 'Risc', 'Fora SLA']} format={(v) => ui.sla(v as JobSla)} />
              <TextArea value={form.description} onChange={(v) => setForm((f) => ({ ...f, description: v }))} label={ui.t('clearIssue')} />
            </div>
            <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button onClick={() => setOpenDialog(false)} className="h-10 rounded-[6px] border border-[#c0c8d5] bg-white px-4 text-[12px] font-semibold text-[#383b3f] dark:border-[#323334] dark:bg-[#08090a] dark:text-[#d0d6e0]">{ui.t('cancel')}</button>
              <button onClick={submit} className="h-10 rounded-[6px] bg-[#e4f222] px-4 text-[12px] font-semibold text-[#08090a]">{ui.t('createSatOrder')}</button>
            </div>
          </div>
        </div>
      ) : null}
...
function Select({ label, value, onChange, values, format = (v) => v }: { label: string; value: string; onChange: (v: string) => void; values: string[]; format?: (v: string) => string }) { return <label className="text-[12px]"><span className="mb-1 block text-[#62666d] dark:text-[#8a8f98]">{label}</span><select value={value} onChange={(e) => onChange(e.target.value)} className="h-10 w-full rounded-[6px] border border-[#c0c8d5] bg-white px-3 text-[13px] outline-none transition focus:border-[#5e6ad2] dark:border-[#323334] dark:bg-[#08090a]">{values.map((v) => <option key={v} value={v}>{format(v)}</option>)}</select></label>; }
...
function OrderMarker({ job }: { job: Job }) { const ui = useSoftwareText(); const icon = job.type === 'Reparacio' ? <Wrench className="h-3 w-3" /> : job.type === 'Manteniment' ? <ClipboardCheck className="h-3 w-3" /> : job.type === 'Muntatge' ? <Hammer className="h-3 w-3" /> : <SearchCheck className="h-3 w-3" />; const c = job.type === 'Reparacio' ? 'border-[#00c2d7]/55 bg-[#00c2d7]/14 text-[#00c2d7]' : job.type === 'Manteniment' ? 'border-[#22c55e]/55 bg-[#22c55e]/12 text-[#22c55e]' : job.type === 'Muntatge' ? 'border-[#6b7cff]/55 bg-[#6b7cff]/14 text-[#6b7cff]' : 'border-[#facc15]/55 bg-[#facc15]/14 text-[#facc15]'; return <Tip text={`${ui.t('type')}: ${ui.type(job.type)}`}><span className={`flex h-5 w-5 items-center justify-center rounded-full border ${c}`}>{icon}</span></Tip>; }
function StatePill({ value }: { value: JobState }) { const ui = useSoftwareText(); const icon = value === 'Completat' ? <Check className="h-3.5 w-3.5" /> : value === 'Blocat' ? <X className="h-3.5 w-3.5" /> : value === 'En curs' ? <Zap className="h-3.5 w-3.5" /> : <Circle className="h-3.5 w-3.5" />; const c = value === 'Completat' ? 'text-[#22c55e]' : value === 'Blocat' ? 'text-[#ff5c5c]' : value === 'En curs' ? 'text-[#00c2d7]' : 'text-[#8a8f98]'; return <Tip text={`${ui.t('status')}: ${ui.state(value)}`}><span className={c}>{icon}</span></Tip>; }
function PriorityPill({ value }: { value: JobPriority }) { const ui = useSoftwareText(); const icon = value === 'Alta' ? <SignalHigh className="h-5 w-5" /> : value === 'Mitja' ? <SignalMedium className="h-5 w-5" /> : <SignalLow className="h-5 w-5" />; const c = value === 'Alta' ? 'text-[#ff5c5c]' : value === 'Mitja' ? 'text-[#facc15]' : 'text-[#6b7cff]'; return <Tip text={`${ui.t('priority')}: ${ui.priority(value)}`}><span className={`inline-flex h-7 w-7 items-center justify-center rounded-[6px] ${c}`}>{icon}</span></Tip>; }
function SlaPill({ value }: { value: JobSla }) { const ui = useSoftwareText(); const c = value === 'OK' ? 'bg-[#22c55e]' : value === 'Risc' ? 'bg-[#facc15]' : 'bg-[#ff5c5c]'; return <Tip text={`SLA: ${ui.sla(value)}`}><span className="inline-flex items-center gap-1.5">{value === 'OK' ? <Check className="h-3.5 w-3.5 text-[#22c55e]" /> : value === 'Risc' ? <Minus className="h-3.5 w-3.5 text-[#facc15]" /> : <X className="h-3.5 w-3.5 text-[#ff5c5c]" />}<span className={`h-1.5 w-1.5 rounded-full ${c}`} /></span></Tip>; }
