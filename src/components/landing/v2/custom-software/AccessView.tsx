...
export function AccessView({ team, jobs, userName, onSetUserName, onAddUser, onToggle, onOpenSat, onOpenCrm, onOpenMaterial }: Props) {
  ...
  if (screen === 'detail' && current) return <WorkerDetail row={current} period={selectedPeriod.label} onBack={() => setScreen('table')} onOpenSat={onOpenSat} onOpenMaterial={onOpenMaterial} />;
  return (
    <motion.div key="access" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="relative h-full overflow-hidden rounded-[10px] border border-[#d0d6e0] bg-white text-[#08090a] dark:border-[#23252a] dark:bg-[radial-gradient(circle_at_20%_0%,rgba(255,255,255,0.055),transparent_26%),linear-gradient(135deg,#111213,#0b0c0d_58%,#101112)] dark:text-[#f7f8f8]">
      <div className="flex min-h-14 flex-col gap-3 border-b border-[#d0d6e0] px-4 py-3 dark:border-[#23252a] lg:flex-row lg:items-center lg:justify-between">
        <div><h3 className="text-[20px] font-semibold leading-tight">Control horari i equip SAT</h3><p className="mt-1 text-[14px] text-[#62666d] dark:text-[#8a8f98]">Treballadors, hores, extres, SATs, permisos i planificacio.</p></div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex rounded-[7px] border border-[#d0d6e0] bg-[#f7f8f8] p-1 dark:border-[#323334] dark:bg-[#08090a]">
            {periodOptions.map((item) => <button key={item.id} onClick={() => setPeriod(item.id)} className={`rounded-[5px] px-3 py-1.5 text-[13px] font-semibold ${period === item.id ? 'bg-[#08090a] text-white dark:bg-[#f7f8f8] dark:text-[#08090a]' : 'text-[#62666d] dark:text-[#8a8f98]'}`}>{item.label}</button>)}
          </div>
          ...
          <button onClick={() => setScreen('calendar')} className="inline-flex h-9 items-center gap-2 rounded-[6px] border border-[#c0c8d5] bg-white px-3 text-[13px] font-semibold dark:border-[#323334] dark:bg-[#08090a]"><CalendarDays className="h-4 w-4" />Calendari</button>
          <button onClick={() => setDialog(true)} className="inline-flex h-9 items-center gap-2 rounded-[6px] bg-[#e4f222] px-3 text-[13px] font-semibold text-[#08090a]"><Plus className="h-4 w-4" />Afegir</button>
        </div>
      </div>
      <div className="min-h-0 overflow-auto">
        <table className="w-full min-w-[860px] text-left text-[14px]">
          <thead className="sticky top-0 z-10 border-b border-[#d0d6e0] bg-white/95 text-[12px] uppercase tracking-[0.08em] text-[#8a8f98] backdrop-blur dark:border-[#23252a] dark:bg-[#111213]/95">
            <tr><Th>Treballador</Th><Th>Hores {selectedPeriod.label.toLowerCase()}</Th><Th>Extres</Th><Th>SATs</Th><Th>Mitjana SAT</Th><Th>Permis</Th><Th>Detall</Th></tr>
          </thead>
          <tbody>{filtered.map(({ member, stats }) => (
            <tr key={member.id} onClick={() => { setSelectedId(member.id); setScreen('detail'); }} className="cursor-pointer border-b border-[#d0d6e0]/70 transition hover:bg-[#f4f6fa] dark:border-[#23252a]/80 dark:hover:bg-[#171819]">
              <td className="px-4 py-4"><div className="font-semibold">{member.name}</div><div className="mt-1 text-[13px] text-[#62666d] dark:text-[#8a8f98]">{member.role} · {member.zone}</div></td>
              ...
              <td className="px-4 py-4"><span className="font-semibold">{stats.done}</span><span className="text-[#8a8f98]"> fetes</span><span className="ml-2 text-[#8a8f98]">{stats.active} obertes</span></td>
...
function AddWorkerDialog({ userName, onSetUserName, onClose, onSubmit }: { userName: string; onSetUserName: (v: string) => void; onClose: () => void; onSubmit: (name: string) => void }) {
  ...
  return <div className="absolute inset-0 z-30 flex items-center justify-center bg-white/64 p-4 backdrop-blur-md dark:bg-[#08090a]/72"><div className="w-full max-w-xl rounded-[10px] border border-[#d0d6e0] bg-[#f7f8f8] p-4 shadow-[0_24px_80px_rgba(8,9,10,0.22)] dark:border-[#323334] dark:bg-[#0f1011]"><div className="mb-4 flex items-start justify-between border-b border-[#d0d6e0] pb-3 dark:border-[#23252a]"><div><h4 className="text-[18px] font-semibold">Alta de treballador</h4><p className="mt-1 text-[13px] text-[#62666d] dark:text-[#8a8f98]">Crea la fitxa base, permisos i condicions de jornada.</p></div><button onClick={onClose} className="rounded-[6px] border border-[#c0c8d5] bg-white p-1 dark:border-[#323334] dark:bg-[#08090a]"><X className="h-4 w-4" /></button></div><div className="grid gap-3 sm:grid-cols-2"><Field label="Nom complet" value={draft.name} onChange={(name) => { setDraft((d) => ({ ...d, name })); onSetUserName(name); }} /><Select label="Rol" value={draft.role} onChange={(role) => setDraft((d) => ({ ...d, role: role as Role }))} values={['Tècnic', 'Coordinador', 'Admin']} /><Field label="Zona" value={draft.zone} onChange={(zone) => setDraft((d) => ({ ...d, zone }))} /><Field label="Contracte" value={draft.contract} onChange={(contract) => setDraft((d) => ({ ...d, contract }))} /><Field label="PIN / QR" value={draft.pin} onChange={(pin) => setDraft((d) => ({ ...d, pin }))} /><Field label="Calendari" value="Laborable + guardies" onChange={() => undefined} /></div><div className="mt-4 flex justify-end gap-2"><button onClick={onClose} className="h-10 rounded-[6px] border border-[#c0c8d5] bg-white px-4 text-[13px] font-semibold dark:border-[#323334] dark:bg-[#08090a]">Cancel·lar</button><button onClick={() => draft.name.trim() && onSubmit(draft.name)} className="h-10 rounded-[6px] bg-[#e4f222] px-4 text-[13px] font-semibold text-[#08090a]">Crear fitxa</button></div></div></div>;
}
...
function fallbackJobs(member: Member): Job[] { return [{ id: 'SAT-PLAN', title: 'Planificacio pendent', client: `Ruta ${member.zone}`, state: 'Pendent', priority: 'Mitja', sla: 'OK', technician: member.name, eta: 'Proper torn', type: 'Manteniment', contact: 'Coordinacio', location: member.zone, asset: 'Ruta SAT', description: '', diagnosis: '', resolution: '', materials: [], photos: [] }]; }
...
function Select({ label, value, onChange, values }: { label: string; value: string; onChange: (v: string) => void; values: string[] }) { return <label className="text-[13px]"><span className="mb-1 block text-[#62666d] dark:text-[#8a8f98]">{label}</span><select value={value} onChange={(e) => onChange(e.target.value)} className="h-10 w-full rounded-[6px] border border-[#c0c8d5] bg-white px-3 text-[14px] outline-none dark:border-[#323334] dark:bg-[#08090a]">{values.map((v) => <option key={v}>{v}</option>)}</select></label>; }
