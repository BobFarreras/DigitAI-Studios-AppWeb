  ...
  return (
    <motion.div key="access" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row">
        <input value={userName} onChange={(e) => onSetUserName(e.target.value)} placeholder="Nom nou usuari" className="h-10 flex-1 rounded-[6px] border border-[#c0c8d5] bg-white px-3 text-[13px] dark:border-[#323334] dark:bg-[#08090a]" />
        <button onClick={onAddUser} className="h-10 rounded-[6px] bg-[#e4f222] px-4 text-[12px] font-semibold text-[#08090a]">Crear usuari</button>
      </div>
        ...
        <div className="space-y-2">
          {team.map((u) => (
            <div key={u.id} className="flex items-center justify-between rounded-[6px] border border-[#c0c8d5] bg-white px-3 py-2 dark:border-[#323334] dark:bg-[#08090a]">
              ...
              <button onClick={() => onToggle(u.id)} className={`rounded-[5px] px-2 py-1 text-[11px] ${u.enabled ? 'bg-[#27a644]/20 text-[#27a644]' : 'bg-[#eb5757]/20 text-[#eb5757]'}`}>{u.enabled ? 'Actiu' : 'Bloquejat'}</button>
            </div>
          ))}
        </div>
      </div>
