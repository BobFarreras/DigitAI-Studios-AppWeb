    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid min-h-[clamp(560px,64svh,740px)] grid-cols-12 gap-3">
      <article className="group col-span-12 rounded-[8px] border border-[#d0d6e0] bg-[#eceff4]/82 p-3 saturate-[0.65] transition-all hover:saturate-100 dark:border-[#23252a] dark:bg-[#161718]/88">
        ...
        <div className="grid grid-cols-1 gap-2 text-[11px] lg:grid-cols-3">
          <MetricCard label={copy.revenue} value="€ 128.4k" sub="+8.4% mensual" chart={<RevenueSpark />} />
          <MetricCard label={copy.sla} value="97.2%" sub="31 / 34 dins SLA" chart={<SlaRadar />} />
          <MetricCard label={copy.workload} value="84%" sub="22 / 26 actives" chart={<WorkloadBars />} />
        </div>
      </article>

      <div className="col-span-12 grid gap-3 lg:col-span-8">
        <article className="group rounded-[8px] border border-[#d0d6e0] bg-[#eceff4]/82 p-3 saturate-[0.65] transition-all hover:saturate-100 dark:border-[#23252a] dark:bg-[#161718]/88">
        ...
        <article className="group rounded-[8px] border border-[#d0d6e0] bg-[#eceff4]/82 p-3 saturate-[0.65] transition-all hover:saturate-100 dark:border-[#23252a] dark:bg-[#161718]/88">
          <div className="mb-2 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-[12px]">
              <button onMouseEnter={() => setTrendFocus('inc')} onMouseLeave={() => setTrendFocus('all')} className={`inline-flex items-center gap-2 rounded-[5px] border px-2 py-1 ${trendFocus === 'inc' ? 'border-[#8a8cff] text-[#8a8cff]' : 'border-[#c0c8d5] text-[#8a8f98]'}`}><span className="h-1.5 w-4 rounded-full bg-[#8a8cff]" />Incidències</button>
              <button onMouseEnter={() => setTrendFocus('avg')} onMouseLeave={() => setTrendFocus('all')} className={`inline-flex items-center gap-2 rounded-[5px] border px-2 py-1 ${trendFocus === 'avg' ? 'border-[#87f1c9] text-[#87f1c9]' : 'border-[#c0c8d5] text-[#8a8f98]'}`}><span className="h-1.5 w-4 rounded-full bg-[#87f1c9]" />Temps mig</button>
            </div>
          </div>
          <div className="h-[250px] [filter:saturate(.4)_grayscale(.35)] transition-all duration-500 group-hover:[filter:saturate(1)_grayscale(0)]">
      ...
      <div className="col-span-12 flex flex-col gap-3 lg:col-span-4">
        <article className="group flex-1 rounded-[8px] border border-[#d0d6e0] bg-[#eceff4]/82 p-3 saturate-[0.65] transition-all hover:saturate-100 dark:border-[#23252a] dark:bg-[#161718]/88">
          <div className="mb-2 flex items-center justify-between">
          ...
          <div className="grid h-[calc(100%-28px)] grid-cols-2 gap-2">
            <CrmMetric title="Leads actius" value="18" tone="violet" />
            <CrmMetric title="Clients actius" value="34" tone="blue" />
            <CrmMetric title="Follow-up al dia" value="76%" tone="green" />
            <CrmMetric title="Dades completes" value="91%" tone="cyan" />
          </div>
        </article>
...
function MetricCard({ label, value, sub, chart }: { label: string; value: string; sub: string; chart: ReactNode }) {
  return (
    <div className="rounded-[6px] border border-[#c0c8d5] bg-white p-2 [filter:saturate(.42)_grayscale(.35)] transition-all duration-500 group-hover:[filter:saturate(1)_grayscale(0)] dark:border-[#323334] dark:bg-[#08090a]">
...
function SmallCard({ label, value, pct }: { label: string; value: number; pct: number }) {
  return <div className="rounded-[6px] border border-[#c0c8d5] bg-white p-2 [filter:saturate(.42)_grayscale(.35)] transition-all duration-500 group-hover:[filter:saturate(1)_grayscale(0)] dark:border-[#323334] dark:bg-[#08090a]"><p className="text-[#62666d] dark:text-[#8a8f98]">{label}</p><p className="text-[30px] font-semibold leading-none">{value}</p><p className="text-[12px] text-[#8a8f98]">{pct}% del total</p><div className="h-1.5 rounded-full bg-[#d8dde7] dark:bg-[#23252a]"><div className="h-1.5 rounded-full bg-gradient-to-r from-[#5e6ad2] to-[#a855f7]" style={{ width: `${pct}%` }} /></div></div>;
}
function CrmMetric({ title, value, tone }: { title: string; value: string; tone: 'violet' | 'blue' | 'green' | 'cyan' }) {
  ...
  return <div className="rounded-[6px] border border-[#c0c8d5] bg-white p-2 [filter:saturate(.42)_grayscale(.35)] transition-all duration-500 group-hover:[filter:saturate(1)_grayscale(0)] dark:border-[#323334] dark:bg-[#08090a]"><p className="text-[12px] text-[#8a8f98]">{title}</p><p className="text-2xl font-semibold">{value}</p><div className="mt-1 h-2 rounded-full bg-[#d8dde7] dark:bg-[#23252a]"><div className={`h-2 rounded-full bg-gradient-to-r ${grad}`} style={{ width: `${Math.max(28, Math.min(100, pct))}%` }} /></div></div>;
}
