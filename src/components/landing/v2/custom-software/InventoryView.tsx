...
const columns = [
  ['Material', 'Referència de magatzem i categoria.'],
  ['Stock', 'Unitats físiques, mínim i disponibilitat real.'],
  ['Estat', 'Semàfor de reposició segons mínim definit.'],
  ['Proveïdor', 'Compra habitual i termini estimat.'],
  ['SAT', 'Unitats reservades o comandes obertes.'],
  ['Acció', 'Obre detall o executa moviment ràpid.'],
] as const;
...
export function InventoryView({ material, materialName, onSetMaterialName, onAddMaterial, onIncrement }: Props) {
  ...
  const rows = useMemo(() => material.map((m) => ({ material: m, profile: getMaterialProfile(m, reserved[m.id] ?? 0, ordered[m.id] ?? false) })), [material, ordered, reserved]);
    ...
    <motion.div key="inventory" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="relative h-full overflow-hidden rounded-[10px] border border-[#d0d6e0] bg-white text-[#08090a] dark:border-[#23252a] dark:bg-[radial-gradient(circle_at_20%_0%,rgba(255,255,255,0.055),transparent_26%),linear-gradient(135deg,#111213,#0b0c0d_58%,#101112)] dark:text-[#f7f8f8]">
      <section className="flex h-full flex-col">
        <div className="flex min-h-12 flex-wrap items-center justify-between gap-2 border-b border-[#d0d6e0] px-2 py-2 dark:border-[#23252a]">
          <div className="flex flex-wrap items-center gap-1 text-[12px] font-[560]">{filters.map((f) => <Tab key={f} label={f} active={filter === f} onClick={() => setFilter(f)} />)}</div>
          <div className="flex flex-wrap items-center justify-end gap-1.5">
            <button onClick={() => setMode(mode === 'table' ? 'analytics' : 'table')} className="inline-flex h-8 items-center gap-1.5 rounded-[6px] border border-[#c0c8d5] bg-white px-2 text-[12px] font-semibold dark:border-[#323334] dark:bg-[#08090a]">{mode === 'table' ? <BarChart3 className="h-4 w-4" /> : <Table2 className="h-4 w-4" />}{mode === 'table' ? 'Gràfiques' : 'Taula'}</button>
            <MiniKpi icon={<PackageCheck className="h-3.5 w-3.5 text-[#6b7cff]" />} label="Refs" value={String(material.length)} />
            <MiniKpi icon={<CircleAlert className="h-3.5 w-3.5 text-[#ef4444]" />} label="Crítics" value={String(material.filter((m) => m.state === 'Crític').length)} />
            <MiniKpi icon={<ShoppingCart className="h-3.5 w-3.5 text-[#facc15]" />} label="Comandes" value={String(rows.filter((r) => r.profile.ordered > 0).length)} />
            <MiniKpi icon={<CheckCircle2 className="h-3.5 w-3.5 text-[#22c55e]" />} label="Valor" value={`${Math.round(stockValue)}€`} />
            <label className="hidden h-8 items-center gap-2 rounded-[6px] border border-[#c0c8d5] bg-white px-2 text-[12px] dark:border-[#323334] dark:bg-[#08090a] md:flex"><Search className="h-3.5 w-3.5 text-[#8a8f98]" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar material..." className="w-32 bg-transparent outline-none placeholder:text-[#8a8f98]" /></label>
            <button onClick={() => setOpenDialog(true)} className="inline-flex h-8 items-center justify-center gap-2 rounded-[6px] bg-[#08090a] px-3 text-[12px] font-semibold text-white dark:bg-[#e4f222] dark:text-[#08090a]"><Plus className="h-4 w-4" />Material</button>
          </div>
        </div>
...
function MaterialRow({ row, active, onSelect, onIncrement, onReserve, onDetail }: { row: Row; active: boolean; onSelect: () => void; onIncrement: () => void; onReserve: () => void; onDetail: () => void }) {
  return <tr onClick={onSelect} className={`border-b border-[#d0d6e0]/70 bg-white transition hover:bg-[#f4f6fa] dark:border-[#23252a]/80 dark:bg-transparent dark:hover:bg-[#171819] ${active ? 'bg-[#f4f6fa] dark:bg-[#151617]' : ''}`}>
    ...
    <td className="px-4 py-4"><p className="font-[560]">{row.material.qty} unitats</p><p className="text-[11px] text-[#8a8f98]">mín. {row.material.min} · disp. {row.profile.available}</p></td>
    ...
    <td className="px-4 py-4"><p className="text-[12px]">{row.profile.reserved} reservat</p><p className="text-[11px] text-[#8a8f98]">{row.profile.ordered ? `${row.profile.ordered} demanat` : 'sense comanda'}</p></td>
    <td className="px-4 py-4"><div className="flex items-center gap-1"><SmallAction label="+1" onClick={onIncrement} /><SmallAction label="Res." onClick={onReserve} /><button onClick={(e) => { e.stopPropagation(); onDetail(); }} className="inline-flex h-7 items-center gap-1 rounded-[6px] border border-[#c0c8d5] bg-white px-2 text-[11px] font-semibold dark:border-[#323334] dark:bg-[#08090a]"><ArrowUpRight className="h-3.5 w-3.5" />Detall</button></div></td>
  </tr>;
}
...
function Tab({ label, active, onClick }: { label: Filter; active: boolean; onClick: () => void }) { return <button onClick={onClick} className={`rounded-[5px] px-2 py-1 transition ${active ? 'bg-[#eceff4] text-[#08090a] dark:bg-[#1a1b1d] dark:text-[#f7f8f8]' : 'text-[#62666d] hover:text-[#08090a] dark:text-[#8a8f98] dark:hover:text-[#f7f8f8]'}`}>{label}</button>; }
...
function StockMark({ state }: { state: StockState }) { const icon = state === 'Crític' ? <CircleAlert className="h-4 w-4" /> : state === 'Baix' ? <Clock3 className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />; const cls = state === 'Crític' ? 'text-[#ef4444]' : state === 'Baix' ? 'text-[#ca8a04]' : 'text-[#16a34a]'; return <FloatingTip text={state === 'Crític' ? 'Stock per sota del mínim crític.' : state === 'Baix' ? 'Cal preparar reposició.' : 'Stock suficient.'} className={`inline-flex ${cls}`}>{icon}</FloatingTip>; }
function MaterialDialog({ name, qty, min, category, supplier, contact, price, leadTime, onName, onQty, onMin, onCategory, onSupplier, onContact, onPrice, onLeadTime, onClose, onSubmit }: { name: string; qty: string; min: string; category: string; supplier: string; contact: string; price: string; leadTime: string; onName: (v: string) => void; onQty: (v: string) => void; onMin: (v: string) => void; onCategory: (v: string) => void; onSupplier: (v: string) => void; onContact: (v: string) => void; onPrice: (v: string) => void; onLeadTime: (v: string) => void; onClose: () => void; onSubmit: () => void }) { return <div className="absolute inset-0 z-20 flex items-center justify-center rounded-[8px] bg-white/62 p-4 backdrop-blur-md dark:bg-[#08090a]/70"><div className="w-full max-w-2xl rounded-[10px] border border-[#d0d6e0] bg-[#f7f8f8] p-4 text-[#08090a] shadow-[0_24px_80px_rgba(8,9,10,0.22)] dark:border-[#323334] dark:bg-[#0f1011] dark:text-[#f7f8f8]"><div className="mb-3 flex items-start justify-between gap-3 border-b border-[#d0d6e0] pb-3 dark:border-[#23252a]"><div><h4 className="text-[16px] font-semibold">Afegir material</h4><p className="mt-1 text-[12px] text-[#62666d] dark:text-[#8a8f98]">Crea una referència amb proveïdor, preu i criteri de reposició.</p></div><button onClick={onClose} className="rounded-[6px] border border-[#c0c8d5] bg-white p-1 text-[#62666d] dark:border-[#323334] dark:bg-[#08090a] dark:text-[#8a8f98]"><X className="h-4 w-4" /></button></div><div className="grid gap-3 sm:grid-cols-2"><Field label="Nom material" value={name} onChange={onName} className="sm:col-span-2" /><Field label="Categoria" value={category} onChange={onCategory} /><Field label="Proveïdor" value={supplier} onChange={onSupplier} /><Field label="Contacte proveïdor" value={contact} onChange={onContact} /><Field label="Termini" value={leadTime} onChange={onLeadTime} /><Field label="Preu unitari" value={price} onChange={onPrice} type="number" /><Field label="Stock inicial" value={qty} onChange={onQty} type="number" /><Field label="Stock mínim" value={min} onChange={onMin} type="number" /></div><div className="mt-4 flex justify-end gap-2"><button onClick={onClose} className="h-10 rounded-[6px] border border-[#c0c8d5] bg-white px-4 text-[12px] font-semibold dark:border-[#323334] dark:bg-[#08090a]">Cancel·lar</button><button onClick={onSubmit} className="h-10 rounded-[6px] bg-[#e4f222] px-4 text-[12px] font-semibold text-[#08090a]">Crear material</button></div></div></div>; }
