    ...
    <div className="h-full overflow-auto rounded-[10px] border border-[#d0d6e0] bg-white text-[#08090a] dark:border-[#23252a] dark:bg-[linear-gradient(135deg,#111213,#0b0c0d_58%,#101112)] dark:text-[#f7f8f8]">
      ...
      <section className="grid gap-4 border-b border-[#d0d6e0] px-4 py-4 dark:border-[#23252a] lg:grid-cols-[1fr_330px]">
        ...
        <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-1">
          <Metric icon={<Boxes className="h-4 w-4" />} label={`${ui.t('stock')} / ${ui.t('minStock')}`} value={`${material.qty} / ${material.min}`} />
          <Metric icon={<PackageCheck className="h-4 w-4" />} label={ui.t('reserved')} value={String(profile.reserved)} />
          <Metric icon={<ShoppingCart className="h-4 w-4" />} label={ui.t('order')} value={profile.ordered ? String(profile.ordered) : ui.t('noOrder')} />
        </div>
      </section>
      <div className="grid gap-0 lg:grid-cols-[1fr_300px]">
        <main className="border-b border-[#d0d6e0] p-4 dark:border-[#23252a] lg:border-b-0 lg:border-r">
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            <Metric icon={<Truck className="h-4 w-4" />} label={ui.t('supplier')} value={profile.supplier} />
            <Metric icon={<UserRound className="h-4 w-4" />} label={ui.t('contact')} value={profile.supplierContact} />
            <Metric icon={<CalendarClock className="h-4 w-4" />} label={ui.t('leadTime')} value={profile.leadTime} />
            <Metric icon={<Euro className="h-4 w-4" />} label={ui.t('price')} value={`${profile.unitPrice.toFixed(2)} €`} />
            <Metric icon={<MapPin className="h-4 w-4" />} label={ui.t('location')} value={profile.location} />
          </div>
