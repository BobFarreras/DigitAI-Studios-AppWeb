  ...
  return (
    <header
      className={[
        ...
        isScrolled ? "bg-white/72 shadow-[0_10px_34px_rgba(8,9,10,0.08)] backdrop-blur-xl dark:bg-[#08090a]/68 dark:shadow-[0_10px_34px_rgba(0,0,0,0.26)]" : "bg-transparent shadow-none",
      ].join(" ")}
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          ...
          {sectionLinks.map((item) => (
            <BrandRevealLink key={item.href} href={item.href} label={t(item.key)} active={isHomePath && activeHref === item.href} onClick={handleAnchor} />
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 md:hidden">
            <ThemeToggle />
            <LanguageSwitcher />
            <MobileHeaderMenu onSectionClick={handleAnchor} />
          </div>
