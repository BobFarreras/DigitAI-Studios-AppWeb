...
export function Footer() {
  ...
  const servicesLinks = [
    { label: "Automatitzacions", href: "/#serveis" },
    { label: "Software a mida", href: "/#serveis" },
    { label: "Agents IA", href: "/#serveis" },
  ];
  const companyLinks = [
      ...
      <div className="container mx-auto px-4 md:px-14">
        <div className="mb-5 grid gap-5 md:mb-7 md:grid-cols-2 md:gap-8 lg:grid-cols-[1.7fr_1fr_1fr_1fr]">
          <div className="space-y-3 text-center md:space-y-4 md:text-left">
            ...
            <p className="mx-auto max-w-sm text-sm leading-relaxed text-[#62666d] dark:text-[#8a8f98] md:mx-0">{t("description")}</p>
            ...
            <div className="grid grid-cols-3 gap-2 lg:grid-cols-1 lg:gap-1.5">
              {legalLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="px-1 text-center text-[11px] font-[590] leading-tight text-[#62666d] transition-colors hover:text-[#8b5cf6] dark:text-[#8a8f98] md:text-sm lg:text-left"
                >
                  {link.label}
                </Link>
              ))}
        ...
        <div className="flex items-center justify-center border-t border-[#d0d6e0] pt-5 text-sm text-[#62666d] dark:border-[#23252a] dark:text-[#8a8f98]">
          <p className="text-center">
            © {currentYear} DigitAI Studios. {t("rights_reserved")}
          </p>
        </div>
      </div>
    </footer>
...
function ContactPill({ icon, label, href }: { icon: React.ReactNode; label: string; href?: string }) {
  ...
  const className =
    "inline-flex h-8 max-w-[180px] items-center gap-1.5 rounded-[6px] border border-[#d0d6e0] bg-white/62 px-2 text-[11px] font-[590] text-[#62666d] dark:border-[#323334] dark:bg-[#161718]/70 dark:text-[#8a8f98]";
  return href ? (
    <a href={href} className={`${className} hover:text-[#08090a] dark:hover:text-[#f7f8f8]`}>
      {content}
    </a>
  ) : (
...
function LinkColumn({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div className="space-y-2.5 text-center md:space-y-3 md:text-left">
      <h4 className="text-sm font-bold text-[#08090a] dark:text-[#f7f8f8]">{title}</h4>
      <ul className="space-y-1.5 md:space-y-2">
        {links.map((link) => (
          <li key={link.label}>
            <Link href={link.href} className="group mx-auto flex w-fit items-center gap-2 text-[13px] text-[#62666d] transition-colors hover:text-[#8b5cf6] dark:text-[#8a8f98] md:mx-0 md:text-sm">
              <span className="h-1.5 w-1.5 rounded-full bg-[#8b5cf6]/0 transition-colors duration-300 group-hover:bg-[#8b5cf6]" />
              {link.label}
            </Link>
          </li>
        ))}
